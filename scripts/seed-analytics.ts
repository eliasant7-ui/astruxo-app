 ...
 * Run with: npm run tsx scripts/seed-analytics.ts
 */

import { db } from '../src/server/db/client.js';
import { userSessions, pwaInstallations, users } from '../src/server/db/schema.js';
...
async function seedAnalytics() {
  try {
    ...
    if (allUsers.length === 0) {
      console.error('❌ No users found. Create some users first.');
      process.exit(1);
    }
    ...
    for (let i = 0; i < sessionsToCreate.length; i += batchSize) {
      ...
      process.stdout.write(`   Progress: ${insertedSessions}/${sessionsToCreate.length}\r`);
    }
    
    console.log(`\n✅ Created ${insertedSessions} sessions`);

    // Generate PWA installations (10-30% of users)
    console.log('\n📱 Generating PWA installations...');
    ...
    if (pwaInstallsToCreate.length > 0) {
      ...
      console.log(`✅ Created ${pwaInstallsToCreate.length} PWA installations`);
    }

    console.log('\n🎉 Analytics data seeded successfully!\n');
    console.log('📊 Summary:');
    console.log(`   - Sessions: ${insertedSessions}`);
    console.log(`   - PWA Installs: ${pwaInstallsToCreate.length}`);
