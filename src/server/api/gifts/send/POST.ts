...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    // Cap currentGoalProgress at goalAmount to prevent overflow (if goal is set)
    await db
      .update(streams)
      .set({
        totalGiftsReceived: sql`${streams.totalGiftsReceived} + ${moneyEarned}`,
        currentGoalProgress: sql`CASE 
          WHEN ${streams.goalAmount} IS NOT NULL AND ${streams.goalAmount} > 0 
          THEN LEAST(${streams.goalAmount}, ${streams.currentGoalProgress} + ${gift[0].coinPrice})
          ELSE ${streams.currentGoalProgress} + ${gift[0].coinPrice}
        END`,
      })
